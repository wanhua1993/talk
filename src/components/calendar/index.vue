...
export default {
  ...
  methods: {
    ...
    setDay(item) {
      ...
      item.tipsData
        ? (this.dataText = item.tipsData[0].contest)
        : (this.dataText = "");
    ...
    init() {
      //日期初始化
      this.getNowDate();
      if (this.originDateValue == "" || !this.originDateValue) {
        //未设置初始日期时，默认系统当前日期
    ...
    getDate() {
      ...
      for (var i = 1; i <= this.monthOfDays[this.currentMonth]; i++) {
        //创建当前月份日历数组
      ...
      for (
        var i = this.monthOfDays[this.currentMonth - 1];
        i > this.monthOfDays[this.currentMonth - 1] - this.dayOfWeek + 1;
        i--
      ) {
        //创建当前上一月的部分天数数据
    ...
    vuePicker() {
      ...
      for (var i = 1; i <= 31; i++) {
        var obj = {
          ...
          value: i,
          date:
            that.currentYear + "-" + String(+that.currentMonth + 1) + "-" + i,
          hasTips: false,
          dataTips: null
        };
        that.data3.push(obj);
      ...
      var picker = (that.myPicker = new Picker({
        ...
        selectedIndex: [
          that.currentYear - that.startYear,
          that.currentMonth,
          that.currentDay - 1
        ],
        title: ""
      ...
      picker.on("picker.change", function(index, selectedIndex) {
        ...
        monthDays = new Array(
          31,
          28 + that.isLeap(that.currentYear),
          31,
          30,
          31,
          30,
        ...
        var setDates =
          that.currentYear + "-" + that.currentMonth + "-" + that.currentDay;
